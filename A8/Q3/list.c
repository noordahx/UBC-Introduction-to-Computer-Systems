...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
  ...
  for (int i = 0; i < list_len(in_list); i++) {
    element_t e = 0;
    element_t* a = &e;
    f(a, list_get(in_list, i));
    list_insert(out_list, i, *a);
  }
}
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  // TODO
  int min = list_len(in_list0) < list_len(in_list1)? list_len(in_list0) : list_len(in_list1);
  
  for (int i = 0; i < min; i++) {
    element_t e = 0;
    element_t* a = &e;
    f(a, list_get(in_list0, i), list_get(in_list1, i));
    list_insert(out_list, i, *a);
  }
}
