...
static void *coalesce(struct myheap *h, void *first_block_start) {
  ...
  // check if the next block is in heap range
  if (!is_within_heap_range(h, get_next_block(first_block_start))) return first_block_start;
  // check if the current block and the next block are free;
  if (block_is_in_use(first_block_start) == 1 || block_is_in_use(get_next_block(first_block_start)) == 1) return first_block_start;
  // else coalesce them
  int size = get_block_size(first_block_start) + get_block_size(get_next_block(first_block_start));
...
static int get_size_to_allocate(int user_size) {
  ...
  if (user_size % HEADER_SIZE != 0) {
    user_size = (user_size / HEADER_SIZE + 1) * HEADER_SIZE;
  }
  // Adding extra space for header and footer (8 byte each)
  user_size += 2 * HEADER_SIZE;
  return user_size;
}
...
static void *split_and_mark_used(struct myheap *h, void *block_start, int needed_size) {
  ...
  if ( size - needed_size >= (3 * HEADER_SIZE) ) {
    set_block_header(block_start, needed_size, 1);
    set_block_header(get_next_block(block_start), size - needed_size, 0); 
  } else {
...
void myheap_free(struct myheap *h, void *payload) {
  ...
  coalesce(h, start);
  if (!is_first_block(h, start)) coalesce(h, get_previous_block(start));
}
...
void *myheap_malloc(struct myheap *h, unsigned int user_size) {
  ...
  for (void* blk = h->start; is_within_heap_range(h, blk); blk = get_next_block(blk)) {
    if ((get_payload_size(blk) >= user_size) && (block_is_in_use(blk) == 0)) {
