  ...
  @Override protected void execute () throws InvalidInstructionException, MachineHaltException, RegisterSet.InvalidRegisterNumberException, MainMemory.InvalidAddressException
  {
    switch (insOpCode.get()) {
      ...
      case 0x1: // ld o(rs), rd .......... 1psd  (p = o / 4)
        // TODO
        int address = reg.get(insOp1.get());
        address += insOp0.get()*4;
        int value = mem.readInteger(address);
        reg.set (insOp2.get(), value);
        break;
      case 0x2: // ld (rs, ri, 4), rd .... 2sid
        // TODO
        address = reg.get(insOp0.get());
        address += reg.get(insOp1.get()) * 4;
        value = mem.readInteger(address);
        reg.set(insOp2.get(), value);
        break;
      case 0x3: // st rs, o(rd) .......... 3spd  (p = o / 4)
        // TODO
        address = reg.get(insOp2.get());
        address += insOp1.get()*4;
        value = reg.get(insOp0.get());
        mem.writeInteger(address, value);
        break;
      case 0x4: // st rs, (rd, ri, 4) .... 4sdi
        // TODO
        address = reg.get(insOp1.get());
        address += reg.get(insOp2.get()) * 4;
        value = reg.get(insOp0.get());
        mem.writeInteger(address, value);
        break;
      case 0x6: // ALU ................... 6-sd
	...
	break;
      case 0x7: // sh? $i,rd ............. 7dii
        // TODO
        value = reg.get(insOp0.get());
        byte firstHex = (byte) (insOp1.get() << 4);
        byte secondHex = (byte) (insOp2.get() & 0xf);
        byte ss = (byte) (firstHex | secondHex);
        if (ss > 0){
            reg.set(insOp0.get(), (value << ss));
        } else {
            reg.set(insOp0.get(), (value >> (-ss)));
        }
        break;
